...
export default function CreateLaporanPage() {
  ...
  const [periodeAwal, setPeriodeAwal] = useState(new Date().toISOString().slice(0, 10));
  const [periodeAkhir, setPeriodeAkhir] = useState(new Date().toISOString().slice(0, 10));
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const { error } = await supabase.from("laporan_laundries").insert(payload);
    if (!error) {
  ...
  return (
    <div className="max-w-5xl space-y-6 pb-20 font-sans">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-black text-gray-900 tracking-tight">Buat Laporan Baru</h1>
          <p className="text-gray-500 font-medium mt-1">Lengkapi rincian statistik operasional Anda.</p>
        </div>
      </div>
